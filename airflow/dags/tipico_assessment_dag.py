...
pull_tipico_api_and_populate_stage = BashOperator(
    ...
    bash_command='python3 ../tipico_git/ingestions/tipico_api_pull.py' ,
    dag=dag
)
...
dbt_dim_event = BashOperator(
    ...
    bash_command='cd .. dbt run --select dim_event' ,
    dag=dag
)
...
dbt_dim_participant = BashOperator(
    ...
    bash_command='cd .. && dbt run --select dim_participant' ,
    dag=dag
)
...
dbt_dim_event_dtls = BashOperator(
    ...
    bash_command='cd .. && dbt run --select dim_event_dtls' ,
    dag=dag
)
...
dbt_dim_group= BashOperator(
    ...
    bash_command='cd .. && dbt run --select dim_group' ,
    dag=dag
)
...
dbt_dim_outcome= BashOperator(
    ...
    bash_command='cd .. && dbt run --select dim_outcome' ,
    dag=dag
)
...
dbt_dim_market= BashOperator(
    ...
    bash_command='cd .. && dbt run --select dim_market' ,
    dag=dag
)
# Populate dim_specifier
dbt_dim_specifier= BashOperator(
    task_id='run_dbt_specifier',
    bash_command='cd .. && dbt run --select dim_specifier' ,
    dag=dag
)
